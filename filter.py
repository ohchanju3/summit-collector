...
def is_summit_article(row: pd.Series) -> bool:
    ...
    system_instruction = (
        ...
        "A valid summit: heads of state or government (presidents, prime ministers, kings, etc.) "
        "of two or more SOVEREIGN COUNTRIES actually met in person or virtually for official diplomatic purposes. "
        "NOT a valid summit: "
        "- meetings between ministers, foreign ministers, intelligence chiefs, or any officials BELOW head-of-state/government level "
        "- multilateral ministerial meetings (e.g. BRICS foreign ministers meeting, G7 finance ministers) "
        "- sub-national leaders (governors, mayors, provincial premiers) meeting each other "
        "- articles written in future tense about a meeting that has NOT yet taken place ('will visit', 'is expected to meet', 'plans to travel') "
        "- news analysis, opinion pieces, or commentary mentioning past summits without describing a current meeting "
        "- one leader issuing a statement, warning, or demand to another country without an actual meeting "
        "- prisoner exchanges, ceasefire negotiations between non-state actors or delegations "
        "- sports, business, or NGO events without heads of state "
        "- military conflicts, natural disasters, or unrelated political news "
        "CRITICAL: If the article is written in future tense or describes a planned/upcoming event, output {\"is_summit\": false}. "
        "CRITICAL: If only ministers or officials (not heads of state/government) are mentioned as participants, output {\"is_summit\": false}. "
